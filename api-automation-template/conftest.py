"""全局 pytest fixture：三句柄 + Allure 环境。"""
...
def pytest_addoption(parser):
    parser.addoption(
        "--env",
        default="sit",
        help="运行环境，对应 config.yaml 中 environments 的键名",
    )
...
def api(config) -> RequestHandler:
    """全局请求句柄，session 级复用（不自动鉴权）。用于测鉴权失败/无权限场景。"""
...
def auth_api(config) -> RequestHandler:
    """带单 token 鉴权的请求句柄：登录后自动注入 Authorization 头与 accessToken 查询参数。"""
    handler = RequestHandler(config)
...
def biz_api(config) -> RequestHandler:
    """带业务鉴权的请求句柄：登录 + 业务多头鉴权 + 岗位头（业务接口权限校验）。"""
    handler = RequestHandler(config)
    token = login(handler, config.auth)
